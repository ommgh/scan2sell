...
import  {getImageFromDjango}  from '@/app/api/barcode/route';
...
export const ProductImagesBarcode = () => {
  ...
  const barcodeURL = useRecoilValue(barcodeUrl);
  ...
  const fetchImages = async () => {
    ...
    try {
      const imageUrls = await getImageFromDjango(barcodeURL);
      console.log('Image URLs:', imageUrls.data.image);
      const imagesByBarcode = imageUrls.data.image;
      setPhotoar(imagesByBarcode.slice(0, 4)); // Assuming the API returns more than 4 image URLs
      setError('');
    } catch (error) {
